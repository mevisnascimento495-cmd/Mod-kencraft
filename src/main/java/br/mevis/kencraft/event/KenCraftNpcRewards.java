...
public final class KenCraftNpcRewards {
    ...
    public static void onLivingDeath(LivingDeathEvent event) {
        ...
        if (victim instanceof RinkaEntity) {
            ...
            if (data.race() == Race.HUMAN && data.arfMissionKills() >= 0 && data.arfMissionKills() < 5) {
            ...
            if (data.race() == Race.HUMAN && nextKills >= 0 && nextKills < 5) {
                player.sendSystemMessage(Component.literal("Missão ARF: Rinkas derrotados " + nextKills + "/5."));
            } else if (data.race() == Race.HUMAN && nextKills == 5) {
