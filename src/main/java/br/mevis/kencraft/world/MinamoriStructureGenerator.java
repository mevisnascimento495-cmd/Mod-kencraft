...
public final class MinamoriStructureGenerator {
    ...
    public static void onNewChunk(ChunkEvent.Load event) {
        ...
        level.getServer().execute(() -> placeMinamori(level, chunk.getPos().getMiddleBlockX(), chunk.getPos().getMiddleBlockZ()));
    }

    private static void placeMinamori(ServerLevel level, int centerX, int centerZ) {
        int groundY = level.getHeight(Heightmap.Types.MOTION_BLOCKING_NO_LEAVES, centerX, centerZ) - 1;
        if (groundY < level.getMinBuildHeight() + 3 || groundY > level.getMaxBuildHeight() - HEIGHT - 2) return;
        if (!validGround(level, centerX, groundY, centerZ)) return;
        for (int x = -12; x <= 12; x += 12) for (int z = -7; z <= 7; z += 7) {
            ...
            if (Math.abs(y - groundY) > 2 || !validGround(level, centerX + x, y, centerZ + z)) return;
        }
    ...
    private static void buildCafe(ServerLevel level, BlockPos o) {
        ...
        for (int x = -1; x <= WIDTH; x++) for (int z = -1; z <= DEPTH; z++) set(level, o.offset(x, 6, z), Blocks.SPRUCE_SLAB);
        for (int x = 3; x <= 8; x++) set(level, o.offset(x, 1, 3), Blocks.SPRUCE_PLANKS);
        set(level, o.offset(5, 2, 3), Blocks.BREWING_STAND); set(level, o.offset(6, 2, 3), Blocks.CAULDRON);
        for (int x : new int[]{5, 10, 15, 19}) { set(level, o.offset(x, 1, 8), Blocks.SPRUCE_PLANKS); set(level, o.offset(x, 1, 10), Blocks.SPRUCE_PLANKS); }
        set(level, o.offset(5, 4, 7), Blocks.LANTERN); set(level, o.offset(19, 4, 7), Blocks.LANTERN);
        drawMinamori(level, o.offset(2, 1, -1));
    }
